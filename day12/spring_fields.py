...
def get_permutations(spring_data_string, contiguous_springs):
    perms = 0
...
def calculate_pieces(remaining_spring_string, contiguous_springs, remaining_factor):
    total = 0
    if remaining_factor == 1:
        total += get_permutations(remaining_spring_string, contiguous_springs)
    else:
        ...
        for i in range(needed_length, len(remaining_spring_string)-needed_length+1):
            if remaining_spring_string[i-1] != "." and remaining_spring_string[i] != "#":
                total += get_permutations(remaining_spring_string[:i-1]+"#", contiguous_springs) * calculate_pieces(remaining_spring_string[i+1:], contiguous_springs, remaining_factor-1)
    return total
...
def main(factor):
    ...
    for line in data:
        ...
        perms = calculate_pieces(spring_data_string, contiguous_springs_folded, factor)
